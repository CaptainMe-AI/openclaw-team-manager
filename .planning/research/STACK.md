...
These are locked in by the project constraints. Research confirms they are current and well-suited.
...
**Architecture note:** ActionCable handles server-to-client push (agent events, approval alerts). REST API (jbuilder) handles CRUD operations (create task, approve/deny, settings). This hybrid is standard for Rails+React dashboards.
...
**Note on @dnd-kit/react (v0.3.2):** A new unified package exists that replaces core/sortable/utilities, but it is at v0.3.x -- still pre-1.0 with only 38 npm dependents. Use the stable @dnd-kit/core ecosystem for production. Migrate to @dnd-kit/react when it reaches 1.0.
...
**Why Font Awesome over Lucide:** The design spec explicitly references FA 6.4.0 with specific icon names. Switching to Lucide would require remapping every icon name and accepting visual differences. Font Awesome's React package is tree-shakeable and the free solid set covers all needed icons.
...
**For real-time agent events (WebSocket push):**
...
**For REST API data (CRUD operations):**
...
**For the mock data layer (before Gateway integration):**
...
---
*Stack research for: OpenClaw Team Manager -- AI Agent Management Dashboard*
*Researched: 2026-03-25*
