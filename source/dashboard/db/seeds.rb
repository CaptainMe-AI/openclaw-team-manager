...
puts "Seeding OpenClaw Team Manager data..."
...
agents_data = [
  { name: "docs-writer",    agent_id: "agt_docs01", status: "active",   llm_model: "opus",   workspace: "~/projects/docs",      uptime_since: 2.days.ago },
  { name: "code-reviewer",  agent_id: "agt_code01", status: "active",   llm_model: "sonnet", workspace: "~/projects/api",       uptime_since: 5.days.ago },
  { name: "test-runner",    agent_id: "agt_test01", status: "active",   llm_model: "sonnet", workspace: "~/projects/tests",     uptime_since: 1.day.ago },
  { name: "deploy-bot",     agent_id: "agt_depl01", status: "idle",     llm_model: "opus",   workspace: "~/projects/infra",     uptime_since: 3.days.ago },
  { name: "data-analyzer",  agent_id: "agt_data01", status: "error",    llm_model: "opus",   workspace: "~/projects/analytics", uptime_since: 6.days.ago },
  { name: "legacy-migrator", agent_id: "agt_legc01", status: "disabled", llm_model: "sonnet", workspace: "~/projects/legacy",    uptime_since: nil }
]
...
approval_data = [
  ...
  { title: "Override monthly budget limit to $500", description: "Agent requesting increased budget for large batch job",
    ...
    context: { current_limit_cents: 10000, requested_limit_cents: 50000 },
    agent: agents[2] },
  { title: "Increase daily API spend to $200", description: "Agent needs higher daily limit for data processing",
    approval_type: "budget_override", status: "pending", risk_level: "medium",
    context: { current_limit_cents: 5000, requested_limit_cents: 20000 },
    agent: agents[3] },
  ...
  { title: "Increase batch processing budget to $100", description: "Agent needed extra budget for overnight batch run",
    ...
    context: { current_limit_cents: 5000, requested_limit_cents: 10000 },
  ...
  { title: "Override budget to $2000/month", description: "Agent requested unreasonably high budget override",
    ...
    context: { current_limit_cents: 10000, requested_limit_cents: 200000 },
...
168.times do |hour_offset|
  ...
  active_agents.each do |agent|
    usage_records << {
      agent_id: agent.id,
      input_tokens: rand(5000..50000),
      output_tokens: rand(2000..25000),
  ...
  if day_offset < 5
    usage_records << {
      ...
      input_tokens: rand(3000..30000),
      output_tokens: rand(1500..15000),
...
settings = [
  ...
  { key: "agents.default_budget_cents", value: { amount: 10000 } },
...
puts "Seeded: #{Agent.count} agents, #{Task.count} tasks, #{Approval.count} approvals, " \
